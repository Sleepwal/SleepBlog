  ...
  <div>
    ...
    >
      ...
      <template #name="{ index, row }">
        ...
        <div v-if="row.replyCommentUserName">
          <span style="color: #2c59cb">回复@{{ row.replyCommentUserName }}</span>
        </div>
      </template>
...
import { deleteComment, getCommentListPage, updateComment } from "@/api/comment";
...
const loadData = async () => {
  ...
  let resp = await getCommentListPage({
